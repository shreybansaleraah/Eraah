...
import "./admin.css";
import { fetchAllDonors, updateDonor } from "../../utils/api-factory";

function AllDonorsPage() {
  const navigate = useNavigate();

  // const classes = useStyles();
  //   const { donors } = useSelector((state) => state.ngo);
  ...
  const handleView = (id) => {
    ...
    navigate("/admin/donorDetails", { state: id });
  };
            ...
            <div className="">
              <Pagination
                ...
                count={
                  search === ""
                    ? donors.length % 6 === 0
                      ? parseInt(donors.length / 6)
                      : parseInt(donors.length / 6) + 1
                    : donors.filter((item) =>
                        item.name.toLowerCase().includes(search.toLowerCase())
                      ).length %
                        6 ===
      ...
      >
        ...
        >
          ...
          >
            <AddDonor
              edit={true}
              modalClose={() => setshowEditDonor(false)}
              data={selectedData}
              handleUpdateDonor={handleUpdate}
            />
          </div>
        </div>
      </Modal>
